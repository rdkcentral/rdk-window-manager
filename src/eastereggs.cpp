...
#define RDK_WINDOW_MANAGER_720_EASTER_EGG_FILE "/tmp/rdkwindowmanager720"
...
namespace RdkWindowManager
{
    ...
    EasterEgg::EasterEgg (std::vector<RdkWindowManagerEasterEggKeyDetails>& details, std::string name, uint32_t timeout, std::string actionJson):mKeyDetails(details), mName(name), mTimeout(timeout), mActionJson(actionJson), mCurrentKeyIndex(0), mTotalUsedTime(0.0), mSatisfied(false)
    {
    }
    ...
    {
        mTotalUsedTime += time;
        struct RdkWindowManagerEasterEggKeyDetails& keyToCheck = mKeyDetails[mCurrentKeyIndex];
        ...
        {
            RdkWindowManager::Logger::log(RdkWindowManager::LogLevel::Debug, "Easter Eggs - Matched portion key: %u modifier:%u", keyToCheck.keyCode, keyToCheck.keyModifiers);
    ...
    {
        std::ifstream file720(RDK_WINDOW_MANAGER_720_EASTER_EGG_FILE);
        if (file720.good())
        {
            Logger::log(LogLevel::Information,  "removing 720 restriction");
            remove( RDK_WINDOW_MANAGER_720_EASTER_EGG_FILE );
        }
        else
        {
            Logger::log(LogLevel::Information,  "adding 720 restriction");
            std::ofstream outputFile(RDK_WINDOW_MANAGER_720_EASTER_EGG_FILE);
            outputFile.close();
        }
    ...
    bool EasterEgg::invokeEvent()
    {
        ...
        if (mSatisfied)
        {
            std::vector<std::map<std::string, RdkWindowManager::RdkWindowManagerData>> eventData(1);
            eventData[0] = std::map<std::string, RdkWindowManager::RdkWindowManagerData>();
            eventData[0]["name"] = mName;
            eventData[0]["action"] = mActionJson;
            if (mName == "RDK_WINDOW_MANAGER_FORCE_720")
            {
            ...
            {
                RdkWindowManager::CompositorController::sendEvent("onEasterEgg", eventData);
            }
            ret = true;
        }
        mSatisfied = false;
    ...
    std::vector<RdkWindowManagerEasterEggKeyDetails> EasterEgg::keyDetails()
    {
        return mKeyDetails;
    }
    ...
    void populateEasterEggDetails()
    {
        ...
        const char* easterEggFile = getenv("RDK_WINDOW_MANAGER_EASTER_EGG_FILE");
        if (easterEggFile)
        {
            rapidjson::Document document;
            bool ret = RdkWindowManager::RdkWindowManagerJson::readJsonFile(easterEggFile, document);
            if (false == ret)
            {
                Logger::log(LogLevel::Information,  "RDKWindowManager easter egg file read error : [unable to open/read file (%s)]", easterEggFile);
                return;
            }
            ...
            if (document.HasMember("easterEggs"))
            {
                ...
                if (jsonValue.IsArray())
                {
                    for (rapidjson::SizeType k = 0; k < jsonValue.Size(); k++)
                    {
                        ...
                        std::vector<RdkWindowManagerEasterEggKeyDetails> keyDetailsList;
                        ...
                        for (rapidjson::SizeType j = 0; j < easterEggSequence.Size(); j++)
                        {
                            ...
                            struct RdkWindowManagerEasterEggKeyDetails keyDetail;
    ...
    void addEasterEgg(std::vector<RdkWindowManagerEasterEggKeyDetails>& details, std::string name, uint32_t timeout, std::string actionJson)
    {
    ...
    void getEasterEggs(std::vector<RdkWindowManagerEasterEggDetails>& easterEggs)
    {
        for(int i=0; i<sEasterEggs.size(); i++)
        {
            EasterEgg& easterEgg = sEasterEggs[i];
            RdkWindowManagerEasterEggDetails easterEggOutput;
    ...
    {
        if ((keyCode == RDK_WINDOW_MANAGER_KEY_CTRL) || (keyCode == RDK_WINDOW_MANAGER_KEY_ALT) || (keyCode == RDK_WINDOW_MANAGER_KEY_SHIFT))
        {
           return;
        }
        for (int i=0; i < sEasterEggs.size(); i++)
        ...
        {
            sEasterEggResolveTime = RdkWindowManager::seconds() + 1.0;
        }
    }
