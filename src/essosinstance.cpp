...
static uint8_t modeSettingForDev(uint8_t devType, uint8_t mode)
{
    switch (devType)
    {
        case RdkWindowManager::DeviceType::GenericLinuxInputDev:
            // doesn't have trackpad or slider
            return (RdkWindowManager::TrackPadDisabled | RdkWindowManager::SliderDisabled);

        case RdkWindowManager::DeviceType::Generic_IR:
            // IR device so obviously doesn't have slider or trackpad
            return (RdkWindowManager::TrackPadDisabled | RdkWindowManager::SliderDisabled);
        
        case RdkWindowManager::DeviceType::VNC_Client:
        case RdkWindowManager::DeviceType::FrontPanel:
            return 0xff;
    }
...
{
    uint8_t deviceType = static_cast<uint8_t>(RdkWindowManager::DeviceType::GenericLinuxInputDev);
    uint8_t deviceMode = 0x0;
    uint16_t deviceId = minor(metadata->deviceNumber);

    RdkWindowManager::inputDeviceTypeAndMode(metadata->id.vendor, metadata->id.product, metadata->devicePhysicalAddress, deviceType, deviceMode);
    deviceMode = modeSettingForDev(deviceType, deviceMode);

#ifdef RDK_WINDOW_MANAGER_ENABLE_KEY_METADATA_EXTENDED_SUPPORT_FOR_IR
    if (deviceType == static_cast<uint8_t>(RdkWindowManager::DeviceType::Generic_IR))
    {
        // additional processing for IR
        RdkWindowManager::irDeviceType(metadata->filterCode, deviceType);
    }
#endif
...
static void processKeyEvent(bool pressEvent, unsigned int key, void *metadata)
{
    ...
    flags |= (rightShiftPressed || leftShiftPressed) ? RDK_WINDOW_MANAGER_FLAGS_SHIFT:0;
    flags |= (rightCtrlPressed || leftCtrlPressed) ? RDK_WINDOW_MANAGER_FLAGS_CONTROL:0;
    flags |= (rightAltPressed || leftAltPressed) ? RDK_WINDOW_MANAGER_FLAGS_ALT:0;
    ...
    if (pressEvent)
    {
        RdkWindowManager::EssosInstance::instance()->onKeyPress(mappedKeyCode, mappedFlags, deviceInfo);
    }
    else
    {
        RdkWindowManager::EssosInstance::instance()->onKeyRelease(mappedKeyCode, mappedFlags, deviceInfo);
    }
}
...
{
    RdkWindowManager::EssosInstance::instance()->onPointerMotion(x, y);
}

static void essosPointerButtonPressed(void *userData, int buttonId, int x, int y)
{
    RdkWindowManager::EssosInstance::instance()->onPointerButtonPress(buttonId, x, y);
}

static void essosPointerButtonReleased(void *userData, int buttonId, int x, int y)
{
    RdkWindowManager::EssosInstance::instance()->onPointerButtonRelease(buttonId, x, y);
}
...
{
    RdkWindowManager::EssosInstance::instance()->onDisplaySizeChanged((uint32_t)width, (uint32_t)height);
}
...
namespace RdkWindowManager
{
    ...
    {
        #ifdef RDK_WINDOW_MANAGER_ENABLE_KEYREPEATS
    ...
    void EssosInstance::initialize(bool useWayland)
    {
        ...
        if (mEssosContext)
        {
            ...
            if (mOverrideResolution)
            {
                RdkWindowManager::Logger::log(LogLevel::Information,  "forcing resolution to: %d x %d" , mWidth ,mHeight);
            ...
            if ( essosError )
            {
                ...
                RdkWindowManager::Logger::log(LogLevel::Information,  "Essos error during initialization: %s", errorDetail);
            }
        }
#ifdef ENABLE_ERM
        gEssRMgr = EssRMgrCreate();
        RdkWindowManager::Logger::log(LogLevel::Information,  "EssRMgrCreate %s",(gEssRMgr != nullptr)?"succeeded":"failed");
#else
        RdkWindowManager::Logger::log(LogLevel::Error,  "ENABLE_ERM not defined");
#endif
    }
    ...
    void EssosInstance::configureKeyInput(uint32_t initialDelay, uint32_t repeatInterval)
    {
        ...
        RdkWindowManager::Logger::log(LogLevel::Information,  "key initial delay: %d repeat interval: %d", mKeyInitialDelay ,mKeyRepeatInterval);
    }
    ...
    void EssosInstance::onKeyPress(uint32_t keyCode, unsigned long flags, uint64_t metadata)
    {
        ...
        {
            RdkWindowManager::Logger::log(LogLevel::Information,  "key inputs ignored for press keycode: %d ", keyCode);
            return;
        }		
    ...
    void EssosInstance::onKeyRelease(uint32_t keyCode, unsigned long flags, uint64_t metadata)
    {
        if (mKeyInputsIgnored)
        {
            RdkWindowManager::Logger::log(LogLevel::Information,  "key inputs ignored for release keycode: %d ", keyCode);
            return;
        }
    ...
    void EssosInstance::onPointerButtonPress(uint32_t keyCode, uint32_t x, uint32_t y)
    {
        if (mKeyInputsIgnored)
        {
            RdkWindowManager::Logger::log(LogLevel::Information,  "key inputs ignored for pointer button press keycode: %d ", keyCode);
	        return;
        }
    ...
    void EssosInstance::onPointerButtonRelease(uint32_t keyCode, uint32_t x, uint32_t y)
    {
        if (mKeyInputsIgnored)
        {
            RdkWindowManager::Logger::log(LogLevel::Information,  "key inputs ignored for pointer button release keycode: %d ", keyCode);
	        return;
        }
    ...
    {
        // enabling key repeats will enable keydown events coming from rdkwindowmanager on keyhold till keyup is performed
