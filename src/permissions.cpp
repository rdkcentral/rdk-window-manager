...
namespace RdkWindowManager
{
    ...
    {
        const char* configFilePath = getenv("RDK_WINDOW_MANAGER_CLIENT_PERMISSIONS_CONFIG");
        if (configFilePath == nullptr)
        {
            Logger::log(LogLevel::Warn, "%s: RDK_WINDOW_MANAGER_CLIENT_PERMISSIONS_CONFIG not specified\n", __func__);
            return;
        }
        Logger::log(LogLevel::Information, "Reading Permissions config from: %s", configFilePath);

        rapidjson::Document document;
        bool ret = RdkWindowManagerJson::readJsonFile(configFilePath, document);
        if (ret == false)
        {
