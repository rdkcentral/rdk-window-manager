...
namespace RdkWindowManager
{
    ...
    bool Cursor::load(const std::string& cursorImageName)
    {
        ...
        mCursorImage = std::make_unique<RdkWindowManager::Image>();
        mIsLoaded = mCursorImage->loadLocalFile(cursorImageName, &mWidth, &mHeight);
        if (!mIsLoaded)
        {
            RdkWindowManager::Logger::log(RdkWindowManager::LogLevel::Error, "error loading cursor image: '%s'", cursorImageName.c_str());
        }

        RdkWindowManager::Logger::log(RdkWindowManager::LogLevel::Information, "finish lading cursor '%s', mIsLoaded: %d", cursorImageName.c_str(), mIsLoaded);
    ...
    void Cursor::setPosition(int32_t x, int32_t y)
    {
        ...
        mX = x;
        mY = screenHeight - y;
        mLastUpdateTime = RdkWindowManager::seconds();
    }
    ...
    void Cursor::draw()
    {
        ...
        if (RdkWindowManager::seconds() - mLastUpdateTime < mInactivityDuration)
        {
